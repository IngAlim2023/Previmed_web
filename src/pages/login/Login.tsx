...
const Login: React.FC = () => {
  ...
  useEffect(() => {
    if (isAuthenticated) {
      ...
      if (storedUser) {
        ...
        if (parsedUser.rol.nombreRol === "Médico") navigate("/home/medico");
  ...
  const onSubmit: SubmitHandler<UsuarioCredenciales> = async (data) => {
    try {
      ...
      // ✅ Redirigir según rol
      if (respu.data.rol === "Médico") navigate("/home/medico");
      else if (respu.data.rol === "Asesor") navigate("/home/asesor");
      else if (respu.data.rol === "Administrador") navigate("/usuarios");
      else navigate("/home/paciente");
    } catch (e) {
